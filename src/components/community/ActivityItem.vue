    ...
    switch (this.operation.type) {
        ...
        if (
          ...
          if (parseInt(this.operation.chainId) === 1) {
            this.description =
              ...
              ` ${(sp * this.vestsToSteem).toFixed(
                2
              )} sp to ${delegatee} from ${this.operation.pool.name}`;
          } else {
            this.description =
              (this.showName ? " add" : "Add") +
              ` ${(sp * this.vestsToHive).toFixed(2)} hp to ${delegatee} from ${
                this.operation.pool.name
              }`;
          }
        } else if (
          poolType === 'atom'
      ...
      case "HARVEST":
        ...
        this.description =
          ...
          ).toFixed(2)} ${ctokenSymbol} from pool: ${this.operation.pool.name}`;
        break;
      case "HARVESTALL":
      ...
      case "ADMINADDPOOL":
        ...
        this.description =
          (this.showName ? " creat" : "Create") +
          ` a new pool: ${this.operation.pool.name}`;
        break;
      case "ADMINCLOSEPOOL":
        this.isAdmin = true;
        this.username = "Admin";
        ...
        this.description =
          (this.showName ? " close" : "Close") +
          ` a pool: ${this.operation.pool.name}`;
        break;
      case "ADMINSETRATIO":
        this.isAdmin = true;
        this.username = "Admin";
        ...
        this.description =
          (this.showName ? " reset" : "Reset") + ` the pool ratios.`;
        break;
