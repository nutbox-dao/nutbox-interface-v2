import { TIME_PERIOD, BLOCK_SECOND, PRICES_SYMBOL } from "@/constant";
...
export const UpdateApysOfPool = async () => {
  ...
  const update = async () => {
    try {
      let [price, pools, communities] = await Promise.all([
        getPrices(),
        getAllPools(),
        getAllCommunities()
      ]);
        ...
        if (pool.type === "HomeChainAssetRegistry") {
          ...
          pool.apy =
            (blocksPerYear *
              ...
              (1 - devRatio / 10000) *
              ctokenPrice) /
            ((pool.totalStakedAmount / 1e18) * p);
          return;
        }

        if (
          pool.type === "SteemHiveDelegateAssetRegistry" &&
          pool.assetType === "sp"
        ) {
          ...
          pool.apy =
            (blocksPerYear *
              ...
              (1 - devRatio / 10000) *
              ctokenPrice) /
            ((pool.totalStakedAmount / 1e18) * steemPrice);
          return;
        }
        if (
          pool.type === "SteemHiveDelegateAssetRegistry" &&
          ...
          pool.apy =
            (blocksPerYear *
              ...
              (1 - devRatio / 10000) *
              ctokenPrice) /
            ((pool.totalStakedAmount / 1e18) * hivePrice);
          return;
        }
        if (
          (pool.type === "SubstrateCrowdloanAssetRegistry" ||
            pool.type === "SubstrateNominateAssetRegistry") &&
          pool.chainId === 2
        ) {
          // polkadot
          ...
          pool.apy =
            (blocksPerYear *
              ...
              (1 - devRatio / 10000) *
              ctokenPrice) /
            ((pool.totalStakedAmount / 1e18) * dotPrice);
          return;
        }
        if (
          (pool.type === "SubstrateCrowdloanAssetRegistry" ||
          ...
          pool.apy =
            (blocksPerYear *
              ...
              (1 - devRatio / 10000) *
              ctokenPrice) /
            ((pool.totalStakedAmount / 1e18) * ksmPrice);
          return;
        }
      });
...
const getPrices = async () => {
  ...
  let [binancePrice, tokenPrices] = await Promise.all([
    getPricesOnCEX(),
    getPricesOnDB()
  ]);
  binancePrice = binancePrice.filter(
    p => PRICES_SYMBOL.indexOf(p.symbol) !== -1
  );
  let res = {};
