...
export const subNominators = async () => {
  let subNominators = store.state.rococo.subNominators
  try {
    subNominators()
  } catch (e) {}
...
// 获取用户投票的情况
  const nominators = await api.query.staking.nominators(store.state.polkadot.account.address, async (nominators) => {
    if (!nominators.toJSON()) {
      store.commit('rococo/saveNominators', [])
      store.commit('rococo/saveLoadingStaking', false)
      return;
    }
    // 获取节点的昵称
    ...
    store.commit('rococo/saveLoadingStaking', false)
    store.commit('rococo/saveNominators', infos)
  })
  store.commit('rococo/saveSubNominators', subNominators)
}
